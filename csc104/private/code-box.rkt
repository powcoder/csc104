...
(define classname-module (format "~s" '(lib "code-snip.ss"
                                            "csc104")))

(define-values (border-width border-style) (values .5 'hilite))
(define-values (left-right-margin top-bottom-margin) (values #;(+ #;8 #;8 4) 6
                                                             #;(+ #;4 4) 2))
...
(define -snip%
  (class* editor-snip% (readable-snip<%>)
    ...
    (define/override (draw dc x y left top right bottom dx dy draw-caret)
      ...
      (local-context dc ([get-pen       set-pen (get-border-pen)]
                     ...
                     (send dc draw-rectangle
                           (+ x (unbox bl))
                           (+ y (unbox bt))
                           (clamp-non-negative (- (unbox bw) (unbox bl) (unbox br)))
                           (clamp-non-negative (- (unbox bh) (unbox bt) (unbox bb))))))
    ...
    (define/override (get-extent dc x y [w #f] [h #f] [d #f] [s #f] [l #f] [r #f])
      ...
      (define Δ (+ (unbox s′) top-bottom-margin))
      (update-box! (+ d′ Δ))
      (update-box! (- s′ Δ))
